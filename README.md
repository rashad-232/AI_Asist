...
## Лицензия
MIT
## Остаётся один момент, который не баг, а особенность библиотеки: pyttsx3 на некоторых системах (особенно Windows) может капризничать при многократных вызовах runAndWait() подряд в цикле — если столкнёшься с этим на практике, дай знать, покажу обходной путь.
